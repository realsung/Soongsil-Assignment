...
// make a new decalration for identifier
...
// make a new decalration for dummy identifier
A_ID *makeDummyIdentifier () {
	A_ID *id;
	id = malloc(sizeof(A_ID));
	id->name = "";
...
A_ID *setFunctionDeclaratorSpecifier (A_ID *id, A_SPECIFIER *p) { 
	...
	if (a)
		if (a->kind != ID_FUNC || a->type->expr)
			syntax_error(12, id->name);
		else { // check prototype: parameter and return type
...
// set declarator_list typep and kind based on storage class
...
// set declarator_list ype and kind
